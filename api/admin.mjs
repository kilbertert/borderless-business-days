...
export function runAdmin(arguments_, config = loadConfig(), { createStore: createStore_ = createStore } = {}) {
  ...
  try {
    switch (command) {
      case "init":
        print({ status: "initialized", databasePath: config.databasePath });
        break;
      case "issue": {
        ...
        if (keyFile) {
          ...
          print({ warning: "This is the only time the plaintext API key will be displayed.", apiKey: issued.apiKey, key: issued.record });
        }
        break;
      }
      case "list":
        print({ keys: store.listKeys() });
        break;
      case "show":
        print(store.usage(required(options, "id")));
        break;
      case "suspend":
        print({ key: store.setStatus(required(options, "id"), "suspended") });
        break;
      case "resume":
        print({ key: store.setStatus(required(options, "id"), "active") });
        break;
      case "revoke":
